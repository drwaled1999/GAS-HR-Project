...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    body { margin: 0; background: #ffffff; color: #000; font-family: "Times New Roman", Times, serif; font-size: 11px; line-height: 1.2; }
    ...
    td { border: 1px solid #000; padding: 5px 6px; vertical-align: middle; font-size: 10.5px; }
    ...
    .qms-title { font-size: 11px; font-weight: 700; padding: 5px 0; border-bottom: 1px solid #000; }
    .form-title { font-size: 21px; font-weight: 700; padding: 4px 0; border-bottom: 1px solid #000; letter-spacing: 0.5px; }
    .form-code { font-size: 11px; font-weight: 700; padding: 4px 0; }
    .meta-label { width: 13%; font-weight: 700; font-size: 10px; padding: 4px 6px; }
    .meta-value { width: 12%; text-align: center; font-size: 10px; padding: 4px 6px; }
    .notice-top { text-align: center; font-family: Arial, Helvetica, sans-serif; font-size: 7.5px; font-weight: 700; padding: 5px 0; }
    .field-label { font-weight: 700; text-transform: uppercase; background: #f3f3f3; font-size: 10px; }
    ...
    .section-title { text-align: center; font-weight: 700; text-transform: uppercase; background: #f3f3f3; font-size: 11px; padding: 4px; }
    ...
    .comments-box { height: 52px; vertical-align: top; font-size: 10px; padding: 6px; }
    .signature-title { background: #f3f3f3; font-weight: 700; text-transform: uppercase; padding: 4px 6px; }
    .signature-name { height: 60px; text-align: center; vertical-align: middle; font-weight: 700; }
    .footer { position: absolute; left: 0; right: 0; bottom: -6mm; display: flex; justify-content: space-between; font-family: Arial, Helvetica, sans-serif; font-size: 7.5px; font-weight: 700; border-top: 1px solid #000; padding-top: 4px; }
...
<body>
  <div class="page">
    <table class="header-table">
      <tr>
        <td rowspan="3" class="logo-cell"><div class="logo-container"><img class="gas-logo-img" src="${logoDataUri}" alt="GAS Logo" /></div></td>
        ...
        <td class="meta-label">Date Revised</td><td class="meta-value">31-05-2026</td>
      </tr>
      <tr><td class="meta-label">Effective Date</td><td class="meta-value">31-05-2026</td></tr>
      <tr><td class="meta-label">Rev./Issue No.</td><td class="meta-value">01 / 00</td></tr>
      <tr><td class="no-border"></td><td class="no-border"></td><td class="meta-label">Page No.</td><td class="meta-value">1 of 1</td></tr>
    </table>
    <div class="notice-top">ANY MODIFICATION TO THIS DOCUMENT SHALL BE THROUGH CQMC ONLY. THINK DIGITAL. WORK SMART. SAVE PLANET.</div>
    <table>
      <tr>
        <td class="field-label" style="width: 20%;">EMPLOYEE NO.</td><td class="field-value center" style="width: 30%;">${escapeHtml(form?.employeeGasId)}</td>
        <td class="field-label" style="width: 20%;">DIVISION</td><td class="field-value" style="width: 30%;">${escapeHtml(form?.projectName || "LEAVE")}</td>
      </tr>
      <tr>
        <td class="field-label">EMPLOYEE NAME</td><td class="field-value">${escapeHtml(form?.employeeName)}</td>
        <td class="field-label">POSITION</td><td class="field-value">${escapeHtml(form?.position || "")}</td>
      </tr>
    </table>
    <div class="spacer"></div>
    <table>
      <tr>
        <td class="field-label" style="width: 16.6%;">REQUEST DATE</td><td class="center" style="width: 16.6%;">${formatDate(form?.requestDate)}</td>
        <td class="field-label" style="width: 20%;">LEAVE START DATE</td><td class="center" style="width: 16.6%;">${formatDate(form?.startDate)}</td>
        <td class="field-label" style="width: 18%;">LEAVE END DATE</td><td class="center" style="width: 13.6%;">${formatDate(form?.endDate)}</td>
      </tr>
      <tr>
        ...
        <td colspan="5" style="padding: 6px 12px;">
          ...
          <div style="margin-top: 6px;">
            ...
            <span style="border-bottom: 1px solid #000; padding-right: 120px; font-weight: bold; margin-left: 4px;">${isSick ? "SICK LEAVE" : ""}</span>
          </div>
        </td>
      </tr>
    </table>
    <div class="spacer"></div>
    <table>
      <tr><td class="section-title" colspan="2" style="width: 50%;">LEAVE DETAILS <em>(TO BE FILLED BY REQUESTOR)</em></td><td class="section-title" colspan="2" style="width: 50%;">REVIEW AND COMMENTS <em>(TO BE FILLED BY HRPM)</em></td></tr>
      <tr>
        <td class="field-label" style="width: 33%;">TOTAL VACATION BALANCE</td><td class="center" style="width: 17%;">${escapeHtml(balance.total)}</td>
      ...
      <tr><td class="field-label">VACATION SALARY</td><td class="center"><span style="margin-right: 8px;"><span class="chk-box">☐</span> YES</span><span><span class="chk-box">☐</span> NO</span></td></tr>
      <tr>
        <td class="field-label">EXIT RE-ENTRY</td><td class="center"><span style="margin-right: 8px;"><span class="chk-box">☐</span> YES</span><span><span class="chk-box">☐</span> NO</span></td>
        <td colspan="2" rowspan="2" class="comments-box"><div class="bold" style="margin-bottom: 2px;">Comments/Justification:</div><div style="font-weight: normal; white-space: pre-wrap;">${escapeHtml(form?.note || "")}</div></td>
      </tr>
      <tr><td class="field-label">TICKET</td><td class="center"><span style="margin-right: 8px;"><span class="chk-box">☐</span> YES</span><span><span class="chk-box">☐</span> NO</span></td></tr>
    </table>
    <div class="spacer"></div>
    <table>
      <tr><td class="field-label" style="width: 25%;">TRAVEL DETAILS</td><td class="field-label center" style="width: 25%;">DATE</td><td class="field-label center" style="width: 25%;">FROM</td><td class="field-label center" style="width: 25%;">TO</td></tr>
    ...
    <table>
      ...
      <tr><td class="field-label center" style="width: 33.3%;">REQUESTED BY</td><td class="field-label center" style="width: 33.3%;">ACKNOWLEDGE BY</td><td class="field-label center" style="width: 33.3%;">APPROVED BY</td></tr>
      <tr><td class="signature-name">${escapeHtml(form?.requestedByName || form?.employeeName)}</td><td class="signature-name"></td><td class="signature-name">${escapeHtml(form?.reviewerName || "")}</td></tr>
    </table>
    <div class="footer"><span>ANY MODIFICATION TO THIS DOCUMENT SHALL BE THROUGH CQMC ONLY.</span><span>THINK DIGITAL. WORK SMART. SAVE PLANET.</span></div>
  </div>
</body>
</html>`;
}

// 1. مسار جلب قائمة النماذج
router.get("/", async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT lr.id AS "requestId", lr.employee_id AS "employeeId", COALESCE(lr.employee_gas_id, e.gas_id) AS "employeeGasId",
              COALESCE(lr.employee_name, e.full_name) AS "employeeName", COALESCE(e.project_name, '') AS "projectName",
              COALESCE(e.package_name, '') AS "packageName", COALESCE(e.job_title, '') AS "position", lr.type, lr.status,
              lr.start_date AS "startDate", lr.end_date AS "endDate", lr.created_at AS "requestDate",
              lr.reviewer_name AS "reviewerName", lr.reviewed_at AS "reviewedAt", lf.id AS "formId", lf.generated_at AS "generatedAt"
       FROM leave_requests lr
       LEFT JOIN employees e ON e.id = lr.employee_id
       LEFT JOIN leave_forms lf ON lf.request_id = lr.id
    ...
    return res.json({ forms, stats: { total: forms.length } });
  } catch (error) {
    return res.status(500).json({ message: error.message || "Failed to load leave forms" });
  }
});

// 2. مسار إضافة استمارة إجازة جديدة بالكامل (CREATE)
router.post("/", async (req, res) => {
  try {
    if (!canViewLeaveForms(req.user)) {
      return res.status(403).json({ message: "Unauthorized to add leave forms" });
    }
    const { employeeGasId, employeeName, type, startDate, endDate, status, note, projectName, position } = req.body;

    // إدخال ريكويست وهمي أو حقيقي في جدول leave_requests لتوليد النموذج عليه
    const reqResult = await query(
      `INSERT INTO leave_requests (employee_gas_id, employee_name, type, start_date, end_date, status, note, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW()) RETURNING id`,
      [employeeGasId, employeeName, type, startDate, endDate, status || "Approved", note || ""]
    );
...
// 3. مسار تعديل استمارة إجازة (UPDATE)
  ...
  try {
    ...
    await query(
      `UPDATE leave_requests 
       SET employee_gas_id = $1, employee_name = $2, type = $3, start_date = $4, end_date = $5, status = $6, note = $7, updated_at = NOW()
       WHERE id = $8::integer`,
      [employeeGasId, employeeName, type, startDate, endDate, status, note, toNullableInteger(requestId)]
    );
...
// 4. مسار حذف استمارة إجازة نهائياً (DELETE)
router.delete("/:requestId", async (req, res) => {
  try {
    ...
    const { requestId } = req.params;
    const safeId = toNullableInteger(requestId);

    // الحذف من الجدولين المرتبطين بالطلب
...
// 5. مسار جلب تفاصيل المعاينة وعرض الـ HTML
...
// 6. مسار طباعة ملف الـ PDF عبر Puppeteer
