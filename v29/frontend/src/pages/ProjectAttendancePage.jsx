...
import {
  uploadProjectAttendanceFile as uploadAttendanceFile,
  getProjectAttendanceSheet as getAttendanceSheet,
  updateProjectAttendanceImportRow as updateAttendanceImportRow,
  approveProjectAttendanceBatch as approveAttendanceBatch,
  reopenProjectAttendanceBatch as reopenAttendanceBatch,
  getAvailableProjectAttendanceUsers as getAvailableAttendanceUsers,
  addUserToProjectAttendanceSheet as addUserToAttendanceSheet,
  excludeUserFromProjectAttendanceSheet as excludeUserFromAttendanceSheet,
  markProjectAttendanceUserStatus as markAttendanceUserStatus,
  directUpdateProjectAttendance as directUpdateAttendance,
  getProjectsForAttendance,
} from "../services/api";
...
export default function ProjectAttendancePage() {
  ...
  const [projectBatchKey, setProjectBatchKey] = useState("");
  const [projectBatchName, setProjectBatchName] = useState("");
  const [attendanceProjects, setAttendanceProjects] = useState([]);
  ...
  const selectedBatchProject = useMemo(() => {
    return safeArray(attendanceProjects).find((project) => String(project?.id) === String(projectBatchKey)) || null;
  }, [attendanceProjects, projectBatchKey]);

  const activeProjectLabel = selectedBatchProject?.name || projectBatchName || projectBatchKey || "No project selected";
  ...
  useEffect(() => {
    ...
    async function loadProjects() {
      try {
        ...
        const result = await getProjectsForAttendance();
        const projects = safeArray(result?.projects);

        if (!mounted) return;

        setAttendanceProjects(projects);

        if (!projectBatchKey && projects.length > 0) {
          const firstProject = projects[0];
          setProjectBatchKey(String(firstProject?.id || ""));
          setProjectBatchName(String(firstProject?.name || firstProject?.code || ""));
        }
      } catch (error) {
        console.error(error);
        if (!mounted) return;
  ...
  async function refreshSheet(currentBatchId) {
    const result = await getAttendanceSheet({ batchId: currentBatchId, projectKey: projectBatchKey });
  ...
  async function loadSelectedMonthSheet() {
    if (!projectBatchKey) {
      setAlert("Please select a project first.", "error");
      return;
    }
  ...
  async function onFileUpload(event) {
    ...
    try {
      ...
      setAvailableUsers([]);
      setAvailableUsersSearch("");
      setAlert("Project attendance file uploaded successfully.");
    } catch (error) {
      console.error(error);
  ...
  async function handleOverrideChange(rowId, overrideType) {
    ...
    try {
      ...
      setAlert("Project attendance row updated successfully.");
    } catch (error) {
      console.error(error);
  ...
  async function handleManualCellChange(row, day, overrideType, manualHours = null) {
    ...
    try {
      ...
      await refreshSheet(batchId);
      setAlert("Project attendance cell updated successfully.");
    } catch (error) {
      console.error(error);
  ...
  async function handleApprove() {
    ...
    try {
      ...
      await refreshSheet(batchId);
      setAlert("Project attendance sheet approved and now visible to employees.");
    } catch (error) {
      console.error(error);
  ...
  async function handleReopen() {
    ...
    try {
      ...
      setAlert("Project attendance sheet reopened. You can edit it now.");
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="attendance-v3-page">
      ...
      {projectsError ? (
        <div className="v3-alert error">{projectsError}</div>
      ) : null}
      ...
      <section className="v3-command-bar">
        ...
        <div className="v3-command-right">
          <div className="v3-filter-group">
            <label>Batch Project</label>
            <select
              value={projectBatchKey}
              onChange={(e) => {
                const value = e.target.value;
                const project = attendanceProjects.find((item) => String(item?.id) === String(value));
                setProjectBatchKey(value);
                setProjectBatchName(project?.name || project?.code || "");
                setBatchId("");
                setAttendanceState({ days: [], rows: [], monthTitle: "Attendance" });
                setMonthName("Attendance");
              }}
      ...
      <section className="v3-overview-grid">
        ...
        <article className="v3-overview-card dark">
          <div>
            ...
            <p>Projects attendance health</p>
          </div>
          <ShieldCheck size={30} />
        </article>
      </section>
      ...
      <section className="v3-status-panel">
        <div>
          ...
          <div>
            <h2>{monthName}</h2>
            <p>
              Project: <strong>{activeProjectLabel}</strong> · Batch: <strong>{batchId || "-"}</strong> · File: <strong>{fileName || "-"}</strong>
            </p>
          </div>
        </div>
              ...
              return (
                <article className="v3-project-card" key={projectName}>
                  <div className="v3-project-top">
                    <div>
                      <h3>{projectName}</h3>
                      <p>Projects attendance analytics and health</p>
                    </div>
