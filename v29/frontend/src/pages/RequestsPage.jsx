...
import { useDevice } from '../src/pages/hooks_useDevice';
...
export default function RequestsPage() {
  ...
  async function loadData() {
    try {
      const [requestResponse, typeResponse, usersResponse] = await Promise.all([
        apiFetch(`/requests-center/list?username=${user.username}`),
        apiFetch('/requests-center/types'),
        apiFetch(`/users?username=${user.username}`)
      ]);
  ...
  const pendingLeaveRequests = useMemo(() => leaveRequests.filter((item) => item.status === 'pending'), [leaveRequests]);
  const pendingAttendanceRequests = useMemo(() => attendanceRequests.filter((item) => item.status === 'pending'), [attendanceRequests]);
  ...
  const mobileQueueItems = useMemo(() => {
    ...
    if (queueFilter === 'pending') merged = merged.filter((item) => item.status === 'pending');
    if (queueFilter === 'salary_transfer') merged = merged.filter((item) => bankFriendlyKeys.has(item.type || item.newStatus));
    if (queueFilter === 'leave') merged = merged.filter((item) => item.queueType === 'leave' && !bankFriendlyKeys.has(item.type || ''));
    if (queueFilter === 'attendance') merged = merged.filter((item) => item.queueType === 'attendance');
    return merged.sort((a, b) => String(b.createdAt || '').localeCompare(String(a.createdAt || '')));
  }, [leaveRequests, attendanceRequests, queueFilter]);

  const reviewQueue = useMemo(() => mobileQueueItems.filter((item) => item.status === 'pending'), [mobileQueueItems]);
  const activeReviewItem = reviewMode ? reviewQueue[0] : null;

  function onFormChange(event) {
    const { name, value, files } = event.target;
    let nextValue = files ? files[0] : value;
    if (name === 'newIban') nextValue = String(value || '').toUpperCase().replace(/[^A-Z0-9]/g, '').replace(/(.{4})/g, '$1 ').trim();
  ...
  async function reviewLeave(id, decision) {
    try {
      await apiFetch(`/requests-center/leave/${id}/review`, {
        method: 'POST',
        body: JSON.stringify({ username: user.username, decision, rejectionReason: decision === 'rejected' ? 'Rejected from mobile review UI' : '' })
      });
  ...
  async function reviewAttendance(id, decision) {
    try {
      ...
      setMessage(decision === 'approved' ? 'تمت الموافقة على طلب تعديل الحضور' : 'تم رفض طلب تعديل الحضور');
      loadData();
    } catch (err) {
      setError(err.message);
    }
      ...
      <div className="mobile-page mobile-admin-requests-page">
        <section className="card request-hero-card">
          ...
          <div className="mobile-tab-row">
            <button className={`tab-pill ${mobileTab === 'queue' ? 'active' : ''}`} onClick={() => setMobileTab('queue')}>Approval Queue</button>
            <button className={`tab-pill ${mobileTab === 'new' ? 'active' : ''}`} onClick={() => setMobileTab('new')}>New Request</button>
          </div>
        </section>
          ...
          <>
            <section className="card mobile-list-card quick-review-shell">
              <div className="page-header compact">
                ...
                <button className="ghost" onClick={() => setReviewMode((prev) => !prev)}>{reviewMode ? 'Exit Review' : 'Quick Review'}</button>
              </div>
                  ...
                  {activeReviewItem ? (
                    <>
                      <div className="request-title-row">
                        ...
                        <span className={`soft-badge ${getStatusClass(activeReviewItem.status)}`}>{activeReviewItem.queueType}</span>
                      </div>
                      <p>{activeReviewItem.type || activeReviewItem.newStatus}</p>
                      <div className="request-detail-grid" style={{ marginTop: 12 }}>
                        <div><span>Status</span><strong>{activeReviewItem.status}</strong></div>
                        <div><span>Date</span><strong>{activeReviewItem.date || activeReviewItem.startDate || '-'}</strong></div>
                        {activeReviewItem.note ? <div className="span-2"><span>Note</span><strong>{activeReviewItem.note}</strong></div> : null}
                        {activeReviewItem.reason ? <div className="span-2"><span>Reason</span><strong>{activeReviewItem.reason}</strong></div> : null}
                      </div>
                      <div className="review-mode-actions">
                        {activeReviewItem.queueType === 'leave' ? (
                          <>
                            <button onClick={() => reviewLeave(activeReviewItem.id, 'approved')}>Approve</button>
                            <button className="ghost" onClick={() => reviewLeave(activeReviewItem.id, 'rejected')}>Reject</button>
                          </>
                        ) : (
                          <>
                            <button onClick={() => reviewAttendance(activeReviewItem.id, 'approved')}>Approve</button>
                            <button className="ghost" onClick={() => reviewAttendance(activeReviewItem.id, 'rejected')}>Reject</button>
                          </>
                        )}
                      </div>
                    </>
                ...
                {leaveRequests.length ? leaveRequests.map((item) => (
                  <article className="card request-mobile-card-v2" key={`leave-mobile-${item.id}`}>
                    <div className="request-card-top">
                      ...
                      {item.attachmentPath ? <a className="ghost-link" href={getProtectedFileUrl(`/files/request/${item.id}`)} target="_blank" rel="noreferrer">Open Attachment</a> : null}
                    </div>
                    <div className="request-detail-grid">
                      ...
                      <div>
                        ...
                        <strong>{item.startDate}{item.endDate && item.endDate !== item.startDate ? ` → ${item.endDate}` : ''}</strong>
                      </div>
                    ...
                    {canReview && item.status === 'pending' ? (
                      <div className="request-card-actions">
                        ...
                        <button className="ghost" onClick={() => reviewLeave(item.id, 'rejected')}>Reject</button>
                      </div>
                    ) : null}
                  </article>
            ...
            <section className="card mobile-list-card">
              ...
              <div className="mobile-request-list enhanced-request-list">
                {attendanceRequests.length ? attendanceRequests.map((item) => (
                  <article className="card request-mobile-card-v2" key={`att-mobile-${item.id}`}>
                    ...
                    {canReview && item.status === 'pending' ? (
                      <div className="request-card-actions">
                        ...
                        <button className="ghost" onClick={() => reviewAttendance(item.id, 'rejected')}>Reject</button>
                      </div>
                    ) : null}
                  </article>
              ...
              <form className="form-grid mobile-form request-form-enhanced" onSubmit={submitLeaveRequest}>
                <label>
                  ...
                  <select name="employeeId" value={form.employeeId} onChange={onFormChange}>
                    ...
                    {employees.map((employee) => (
                      <option key={employee.id} value={employee.id}>{employee.name} — {employee.gasId}</option>
                    ))}
                  </select>
                </label>
                <label>
                  Or GAS ID
                  <input name="employeeGasId" value={form.employeeGasId} onChange={onFormChange} placeholder="مثال: 2036" />
                </label>
                <label>
                  Request Type
                ...
                {needsBankFields ? (
                  <>
                    <label>
                      Current Bank
                      <input name="currentBank" value={form.currentBank} onChange={onFormChange} placeholder="البنك الحالي" required />
                    </label>
                    <label>
                      New Bank
                      <input name="newBank" value={form.newBank} onChange={onFormChange} placeholder="البنك الجديد" required />
                    </label>
                    <label className="span-2">
                      New IBAN
                      <input name="newIban" value={form.newIban} onChange={onFormChange} placeholder="SA00 0000 0000 0000 0000 0000" required />
                    </label>
                  </>
                ) : null}
                ...
                <label className="span-2">
                  Note
                  <textarea name="note" value={form.note} onChange={onFormChange} rows="4" placeholder="سبب الطلب أو تفاصيل إضافية" />
                </label>
        ...
        {mobileTab === 'queue' ? <button className="floating-review-fab" onClick={() => setReviewMode(true)}>Review Pending</button> : null}
      </div>
    );
  }
    ...
    <div className="page">
      ...
      <div className="grid-two">
        <section className="card">
          ...
          {canCreate ? (
            <form className="form-grid" onSubmit={submitLeaveRequest}>
              ...
              <label className="span-2">
                Note
                <input name="note" value={form.note} onChange={onFormChange} placeholder="سبب الطلب أو أي ملاحظة" />
              </label>
        ...
        <section className="card">
          ...
          <div className="mobile-stat-grid">
            <article className="card mobile-stat info"><span>Pending Leave / Task</span><strong>{pendingLeaveRequests.length}</strong></article>
            <article className="card mobile-stat warning"><span>Pending Attendance</span><strong>{pendingAttendanceRequests.length}</strong></article>
          </div>
          <div className="badge-cluster" style={{ marginTop: 16 }}>
            {types.map((type) => (
              <span key={type.code} className={`soft-badge ${type.requiresAttachment ? 'warning' : ''}`}>{type.label}</span>
            ))}
          </div>
        </section>
      </div>
      ...
      <section className="card table-wrap compact-table">
        <div className="page-header compact"><div><h2>Leave / Task Requests</h2><p className="muted">معاينة المرفقات، البنك، والآيبان عند الحاجة.</p></div></div>
        <table>
          <thead>
            <tr>
              <th>Employee</th>
          ...
          <tbody>
            {leaveRequests.map((item) => (
              <tr key={`leave-${item.id}`}>
                ...
                <td>{item.type}</td>
                <td>{item.startDate}{item.endDate && item.endDate !== item.startDate ? ` → ${item.endDate}` : ''}</td>
                <td><span className={`soft-badge ${getStatusClass(item.status)}`}>{item.status}</span></td>
                <td>
                  {bankFriendlyKeys.has(item.type) ? (
                    <div className="small">
                ...
                <td>{item.attachmentPath ? <a href={getProtectedFileUrl(`/files/request/${item.id}`)} target="_blank" rel="noreferrer">{item.attachmentName || 'Preview'}</a> : <span className="muted small">No attachment</span>}</td>
      ...
      <section className="card table-wrap compact-table">
        <div className="page-header compact"><div><h2>Attendance Adjustment Requests</h2><p className="muted">طلبات تعديل الحضور المرتبطة بالجدول الشهري.</p></div></div>
        <table>
          <thead>
            <tr>
              <th>Employee</th>
