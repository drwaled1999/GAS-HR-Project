...
function responseLabel(value) {
  const map = { pending: "Pending", accepted: "Accepted", declined: "Declined", tentative: "Tentative" };
  ...
  return (
    <div className="employee-meetings-page">
      <style>{`
        ...
        .meeting-link { color:#2563eb; text-decoration:none; font-weight:950; display:inline-flex; align-items:center; gap:7px; }
        ...
        @media (max-width: 640px) { .emp-meetings-hero, .employee-meeting-card { border-radius:22px; padding:16px; } .hero-top, .meeting-head { flex-direction:column; } .hero-count { width:100%; box-sizing:border-box; } .meta-grid { grid-template-columns:1fr; } .actions { display:grid; grid-template-columns:1fr; } .actions button { justify-content:center; } }
      `}</style>
      ...
      <section className="emp-meetings-hero">
        <div className="hero-top">
          <div>
            <h1>My Meetings</h1>
            <p>Review official meeting invitations from the administration and respond directly from your employee portal.</p>
          </div>
      ...
      <div className="meetings-list">
        {!loading && !filteredMeetings.length ? <div className="empty-state">No meeting invitations yet.</div> : null}
        ...
        {filteredMeetings.map((meeting) => (
          <article className="employee-meeting-card" key={meeting.id}>
            <div className="meeting-head">
              <div>
                <h2>{meeting.title}</h2>
                <p style={{ margin: "6px 0 0", color: "#64748b", fontWeight: 850, fontSize: ".82rem" }}>
                  Created by {meeting.createdByName || "Administration"}
                </p>
              </div>
              <span className={`pill ${meeting.responseStatus || "pending"}`}>{responseLabel(meeting.responseStatus)}</span>
            </div>

            {meeting.agenda ? <p className="agenda">{meeting.agenda}</p> : null}

            <div className="meta-grid">
              <div className="meta"><CalendarDays size={18} /><span>{formatDate(meeting.meetingDate)}</span></div>
              <div className="meta"><Clock3 size={18} /><span>{meeting.startTime}{meeting.endTime ? ` - ${meeting.endTime}` : ""}</span></div>
              <div className="meta"><MapPin size={18} /><span>{meeting.location || "No location"}</span></div>
              <div className="meta"><MessageSquareText size={18} /><span>{meeting.status || "scheduled"}</span></div>
            </div>

            {meeting.meetingLink ? (
              <a className="meeting-link" href={meeting.meetingLink} target="_blank" rel="noreferrer">
                Open meeting link <ExternalLink size={16} />
              </a>
            ) : null}
            ...
            <div className="response-box">
              <textarea
                ...
                onChange={(e) => setNoteByMeeting((prev) => ({ ...prev, [meeting.id]: e.target.value }))}
                placeholder="Optional note to administration..."
              />
              <div className="actions">
                <button className="accept" type="button" disabled={savingId === `${meeting.id}-accepted`} onClick={() => respond(meeting.id, "accepted")}>
                  <CheckCircle2 size={17} /> Accept
                </button>
                <button className="tentative" type="button" disabled={savingId === `${meeting.id}-tentative`} onClick={() => respond(meeting.id, "tentative")}>
                  <MessageSquareText size={17} /> Tentative
                </button>
                <button className="decline" type="button" disabled={savingId === `${meeting.id}-declined`} onClick={() => respond(meeting.id, "declined")}>
